...
<Callout type="info">
  The `scan` command exits `0` even if there are findings, unless `--fail-on-findings` is explicitly
  set. This makes it safe to use in report-only mode without affecting pipeline status.
</Callout>
