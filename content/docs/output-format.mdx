...
<Callout type="warn">
  The ranking is counter-intuitive: `safe` findings are the most certain candidates for deletion,
  but have the lowest rank. A higher `--min-confidence` value means more uncertain findings.
</Callout>
